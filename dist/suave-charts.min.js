...
function(c){return document.body.contains(this.svg.container[0][0])?c():setTimeout(function(b){return function(){return b.waitToBeInDom(c)}}(this),100)};d.prototype.draw=function(){if(!this.listenerBound)return window.addEventListener("resize",this.render),this.listenerBound=!0};d.prototype.remove=function(){var c,b;window.removeEventListener("resize",this.render);null!=this.tip&&this.tip.remove();c=this.svg.domElement;for(b=[];c.firstChild;)b.push(c.removeChild(c.firstChild));return b};return d}();var filterTicks$$module$target$coffeescript$filter_ticks,module$target$coffeescript$filter_ticks=filterTicks$$module$target$coffeescript$filter_ticks=function(d,c,b){var a,f;return"ordinal"===b?(a=c.domain(),f=Math.round(a.length/(d-2)),a.filter(function(b,c){return 0===c||b===a[a.length-1]?!0:0===c%f})):c.ticks(d)};var Axes$$module$target$coffeescript$axes,filterTicks$$module$target$coffeescript$axes,bind$$module$target$coffeescript$axes=function(d,c){return function(){return d.apply(c,arguments)}};filterTicks$$module$target$coffeescript$axes=module$target$coffeescript$filter_ticks;
var module$target$coffeescript$axes=Axes$$module$target$coffeescript$axes=function(){function d(c,b,a,f,d){this.xScale=b;this.yScale=a;this.options=f;this.axisLabelPadding=d;this.newAxis=bind$$module$target$coffeescript$axes(this.newAxis,this);this.xAxisSelection=c.append("g").attr("class","x axis");this.yAxisSelection=c.append("g").attr("class","y axis");this.xAxis=this.newAxis(this.xScale,"bottom");this.yAxis=this.newAxis(this.yScale,"left")}d.prototype.draw=function(c,b){var a,d;this.xAxisSelection.attr("transform",
...
a,c,d,e,g,l,m,h;this.svg.resize();this.x.rangeRoundBands([0,this.svg.width]);a={xTransform:"translate(0, "+this.svg.height+")",yTransform:"",groupsTransform:function(a){return function(b){return"translate("+a.x(b)+", 0)"}}(this),xRange:[0,this.svg.width],yRange:[this.svg.height,0],barX:function(a){return function(b){return a.x(b.label)}}(this),barY:function(a){return function(b){return a.y(b.value)}}(this),barWidth:function(a){return function(){return a.x.rangeBand()}}(this),barHeight:function(a){return function(b){return a.svg.height-
...
b.tipPadding)}}(this)).text(function(a){return function(b){return a.options.tooltipFormat(b.value)}}(this));l=this.totalBar;m=this.totalBarTooltip;h=this.y;d=this.sum;g=this.options.tooltipFormat;e=this.tipPadding;c=this.onChangeCallback;b=d3.behavior.drag();b.on("drag",function(b){var c,f;c=d();c=Math.min(Math.round(h.invert(d3.event.y)-b.value),c);c=Math.max(0,Math.min(b.value+c,h.domain()[1]));b.value=c;d3.select(this).attr("cy",h(c));b=d3.select(this.parentNode);b.select("rect").attr("height",
a.barHeight({value:c})).attr("y",a.barY({value:c}));b.select("text").attr("y",function(b){return a.barY(b)-e}).text(g(c));f=d();l.each(function(a){return a.value=f});l.attr("height",a.barHeight({value:f})).attr("y",a.barY({value:f}));return m.attr("y",a.barY({value:f})-e).text(g(f))});b.on("dragend",function(a){if(null!=c)return c({label:a.label,value:a.value,remaining:d()})});return this.dots.attr("cx",function(b){return function(b){return a.barX(b)+.5*a.barWidth()}}(this)).attr("cy",function(b){return function(c){return a.barY(c)+
.5*b.options.dotSize}}(this)).attr("class","drag-handle").call(b)};c.prototype.draw=function(b){c.__super__.draw.call(this);this.onChangeCallback=b.onChangeCallback;return this.waitToBeInDom(function(a){return function(){return a.drawInternal(b)}}(this))};c.prototype.drawInternal=function(b){var a;a=[{value:b.bars.reduce(function(a,b){return a+b},0),label:b.domainLabel}];b.bars.forEach(function(c,d){return a.push({value:c,label:b.labels[d]})});this.y.domain(b.domain);this.x.domain(a.map(function(a){return a.label}));
...
b){return 0!==b});this.bars=this.barGroupsWithoutTotal.append("rect");this.dots=this.barGroupsWithoutTotal.append("circle").attr("r",this.options.dotSize);this.tooltips=this.barGroupsWithoutTotal.append("text").attr("text-anchor","middle");this.totalBarGroup=this.barGroups.filter(function(a,b){return 0===b}).datum({value:this.sum(),label:b.domainLabel}).append("g").attr("class","total");this.totalBarBackground=this.totalBarGroup.append("rect").attr("class","background");this.totalBar=this.totalBarGroup.append("rect").attr("class",
"sum");this.totalBarTooltip=this.totalBarGroup.append("text").attr("text-anchor","middle");return this.render()};return c}(AbstractChart$$module$target$coffeescript$goals_chart);var AbstractChart$$module$target$coffeescript$histogram,Histogram$$module$target$coffeescript$histogram,MarginCalculator$$module$target$coffeescript$histogram,Tooltip$$module$target$coffeescript$histogram,defaultHistogramOptions$$module$target$coffeescript$histogram,bind$$module$target$coffeescript$histogram=function(d,c){return function(){return d.apply(c,arguments)}},extend$$module$target$coffeescript$histogram=function(d,c){function b(){this.constructor=d}for(var a in c)hasProp$$module$target$coffeescript$histogram.call(c,
...
return b.area(a.values)}}(this));return this.dots.attr("cx",function(b){return function(a){return b.x(a.x)}}(this)).attr("cy",function(b){return function(a){return b.y(a.y+a.yTranslation)}}(this))};c.prototype.chooseInterpolation=function(b){return b.smooth?"monotone":"linear"};c.prototype.formatData=function(b,a){var c,d,e,g,l,m,h,k,p,q,z,A,r,t,y,u,v,w,x;null==a&&(a=!1);l=this.isTimeSeries&&"number"===typeof b.labels[0]?b.labels.map(function(a){return new Date(a)}):b.labels;v=u=r=t=null;p=[];h=b.lines;
e=c=0;for(m=h.length;c<m;e=++c)for(g in k=h[e],p.push({values:Array(k.values.length)}),k)q=k[g],"values"!==g&&(p[e][g]=q);e=q=0;for(m=l.length;q<m;e=++q)for(g=l[e],w=c=0,A=b.lines,d=z=0,h=A.length;z<h;d=++z){k=A[d];y=k.values[e];x=y+w;if(null==t||t>g)t=g;if(null==r||r<g)r=g;if(null==v||v>x)v=x;if(null==u||u<x)u=x;d=p[d];null!=k.baselineValues&&(c=k.baselineValues[e]);d.values[e]={x:g,y:y,yTranslation:w,areaY0:c,lineKey:k.label};a&&(w+=y)}return{labels:l,lines:p,xDomain:[t,r],yDomain:["log"===this.options.yScale?
