...
this.xScale),this.xAxis.tickValues(a));this.xAxisSelection.call(this.xAxis);return this.yAxisSelection.call(this.yAxis)};d.prototype.newAxis=function(c,b){return d3.svg.axis().scale(c).orient(b).tickPadding(this.axisLabelPadding)};return d}();var MarginCalculator$$module$target$coffeescript$margin_calculator,module$target$coffeescript$margin_calculator=MarginCalculator$$module$target$coffeescript$margin_calculator=function(){function d(c){this.svg=c}d.prototype.calcLeftMargin=function(c,b,a,d){var f;null==a&&(a="y");null==d&&(d=20);a=this.svg.chart.append("g").attr("class",a+" axis").attr("opacity",0);a.call(c);c=a.selectAll("text").sort(function(a,b){return b.toString().length-a.toString().length});b=1<=c.length&&1<=c[0].length?(f=c[0][0].getBBox(),
...
for(d=v.length;c<d;c++)a=v[c],e.push("[object Array]"===Object.prototype.toString.call(a)?a:[a]);return e}();this.y.domain([0,d3.max(d3.merge(d,function(a){return d3.max(a)}))]);this.x.domain(b.labels);this.groupedX.domain(d[0].map(function(a,b){return b}));null!=this.options.ticks&&0<this.options.ticks&&this.xAxis.tickValues(filterTicks$$module$target$coffeescript$bar_chart(this.options.ticks,this.x));"vertical"===this.options.layout?this.options.margin.left=this.calc.calcLeftMargin(this.yAxis,this.options.margin.left):
(this.x.rangeRoundBands([0,10],.1),this.options.margin.left=this.calc.calcLeftMargin(this.xAxis,this.options.margin.left,"x"));this.xAxisSelection=this.svg.chart.append("g").attr("class","x axis");this.yAxisSelection=this.svg.chart.append("g").attr("class","y axis");this.groups=this.svg.chart.selectAll(".barGroup").data(b.labels).enter().append("g").attr("class",function(a,b){return"barGroup barGroup-"+b+" "+a});this.bars=this.groups.selectAll(".bar").data(function(a,b){return d[b]}).enter().append("rect").attr("class",
