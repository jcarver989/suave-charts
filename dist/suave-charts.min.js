...
a.barWidth).attr("height",a.barHeight);this.totalBarBackground.attr("x",a.barX).attr("y",0).attr("width",a.barWidth).attr("height",this.y.range()[0]);b=function(b){return function(b){return a.barX(b)+.5*a.barWidth()}}(this);f=function(b){return function(c){return Math.min(a.barY(c)-b.tipPadding,b.svg.height-b.tipPadding)}}(this);this.tooltips.attr("x",b).attr("y",f);this.tooltips.append("tspan").attr("x",b).attr("y",f).attr("dy","-1.5em").text(function(a){return function(b){return a.options.tooltipFormat(b.value)}}(this));
this.tooltips.append("tspan").attr("class","sub-label").attr("x",b).attr("y",f).attr("dy","0em").text(function(a){return function(a){return a.tooltipLabel}}(this));this.totalBarTooltip.attr("x",function(b){return function(b){return a.barX(b)+.5*a.barWidth()}}(this)).attr("y",function(b){return function(c){return Math.min(a.barY(c)-b.tipPadding,b.svg.height-b.tipPadding)}}(this)).text(function(a){return function(b){return a.options.tooltipFormat(b.value)}}(this));l=this.totalBar;m=this.totalBarTooltip;
g=this.y;d=this.sum;k=this.options.tooltipFormat;e=this.tipPadding;c=this.onChangeCallback;b=d3.behavior.drag();b.on("drag",function(b){var c,h;c=d();c=Math.min(Math.round(g.invert(d3.event.y)-b.value),c);c=Math.max(0,Math.min(b.value+c,g.domain()[1]));b.value=c;d3.select(this).selectAll("circle").attr("cy",g(c));b=d3.select(this.parentNode);b.select("rect").attr("height",a.barHeight({value:c})).attr("y",a.barY({value:c}));b.select("text").attr("y",function(b){return a.barY(b)-e}).attr("dy","-1.5em").text(k(c));
...
this.onChangeCallback=b.onChangeCallback;return this.waitToBeInDom(function(a){return function(){return a.drawInternal(b)}}(this))};c.prototype.drawInternal=function(b){var a,c;a=[{value:b.bars.reduce(function(a,b){return a+b},0),label:b.domainLabel}];c=null!=b.tooltipLabels?b.tooltipLabels:function(){var a,c,d;c=b.labels;d=[];a=0;for(c=c.length;a<c;a++)d.push("");return d}();b.bars.forEach(function(d,e){return a.push({value:d,label:b.labels[e],tooltipLabel:c[e]})});this.y.domain(b.domain);this.x.domain(a.map(function(a){return a.label}));
this.options.margin.left=this.calc.calcLeftMargin(this.yAxis,this.options.margin.left);this.xAxisSelection=this.svg.chart.append("g").attr("class","x axis");this.yAxisSelection=this.svg.chart.append("g").attr("class","y axis");this.groups=this.svg.chart.selectAll(".barGroup").data(a).enter().append("g").attr("class",function(a,b){return"barGroup barGroup-"+b+" "+a.label});this.barGroups=this.groups.append("g").attr("class",function(a,b){return"bar bar-"+b});this.barGroupsWithoutTotal=this.barGroups.filter(function(a,
b){return 0!==b});this.bars=this.barGroupsWithoutTotal.append("rect");this.dotGroups=this.barGroupsWithoutTotal.append("g").attr("class","drag-handle-group");this.dots=this.dotGroups.append("circle").attr("r",this.options.dotSize);this.innerDots=this.dotGroups.append("circle").attr("r",this.options.dotSize/2);this.tooltips=this.barGroupsWithoutTotal.append("text").attr("text-anchor","middle");this.totalBarGroup=this.barGroups.filter(function(a,b){return 0===b}).datum({value:this.sum(),label:b.domainLabel}).append("g").attr("class",
"total");this.totalBarBackground=this.totalBarGroup.append("rect").attr("class","background");this.totalBar=this.totalBarGroup.append("rect").attr("class","sum");this.totalBarTooltip=this.totalBarGroup.append("text").attr("text-anchor","middle");return this.render()};return c}(AbstractChart$$module$target$coffeescript$goals_chart);var AbstractChart$$module$target$coffeescript$histogram,Histogram$$module$target$coffeescript$histogram,MarginCalculator$$module$target$coffeescript$histogram,Tooltip$$module$target$coffeescript$histogram,defaultHistogramOptions$$module$target$coffeescript$histogram,bind$$module$target$coffeescript$histogram=function(d,c){return function(){return d.apply(c,arguments)}},extend$$module$target$coffeescript$histogram=function(d,c){function b(){this.constructor=d}for(var a in c)hasProp$$module$target$coffeescript$histogram.call(c,
