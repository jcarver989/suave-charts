...
this.yAxis=d3.svg.axis().scale(this.y).orient("left").tickFormat(this.options.yLabelFormat);window.addEventListener("resize",this.render)}extend$$module$target$coffeescript$bar_chart(a,c);a.prototype.render=function(){this.svg.resize();this.options.grid&&this.yAxis.tickSize(-this.svg.width);this.x.rangeRoundBands([0,this.svg.width],.1);this.groupedX.rangeRoundBands([0,this.x.rangeBand()],this.options.barSpacing);this.y.range([this.svg.height,0]);this.xAxisSelection.attr("transform","translate(0, "+
this.svg.height+")").call(this.xAxis);this.yAxisSelection.call(this.yAxis);this.groups.attr("transform",function(a){return function(b){return"translate("+a.x(b)+", 0)"}}(this));return this.bars.attr("x",function(a){return function(b,c){return a.groupedX(c)}}(this)).attr("y",function(a){return function(b){return a.y(b)}}(this)).attr("width",this.groupedX.rangeBand()).attr("height",function(a){return function(b){return a.svg.height-a.y(b)}}(this))};a.prototype.draw=function(a){var b,c,e,f;c=function(){var c,
e,g,f;g=a.bars;f=[];c=0;for(e=g.length;c<e;c++)b=g[c],f.push("[object Array]"===Object.prototype.toString.call(b)?b:[b]);return f}();this.y.domain([0,d3.max(d3.merge(c,function(a){return d3.max(a)}))]);this.x.domain(a.labels);this.groupedX.domain(c[0].map(function(a,b){return b}));this.options.margin.left=this.calc.calcLeftMargin(this.yAxis,this.options.margin.left);this.xAxisSelection=this.svg.chart.append("g").attr("class","x axis");this.yAxisSelection=this.svg.chart.append("g").attr("class","y axis");
this.groups=this.svg.chart.selectAll(".bar-group").data(a.labels).enter().append("g").attr("class",function(a){return"bar-group "+a});this.bars=this.groups.selectAll(".bar").data(function(a,b){return c[b]}).enter().append("rect").attr("class",function(a,b){return"bar bar-"+b});this.options.tooltips&&(f=this.options.tooltipFormat,e=this.tip||(this.tip=new Tooltip$$module$target$coffeescript$bar_chart(document)),this.bars.on("mouseover",function(a){e.html(f(a));return e.show(this)}),this.bars.on("mouseout",
