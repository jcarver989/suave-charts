...
this.y=d3.scale.linear();this.xAxis=d3.svg.axis().scale(this.x).orient("bottom").tickFormat(this.options.xLabelFormat);this.yAxis=d3.svg.axis().scale(this.y).orient("left").tickFormat(this.options.yLabelFormat);window.addEventListener("resize",this.render)}extend$$module$target$coffeescript$bar_chart(a,c);a.prototype.remove=function(){var b,d;window.removeEventListener("resize",this.render);b=this.svg.domElement;for(d=[];b.firstChild;)d.push(b.removeChild(b.firstChild));return d};a.prototype.render=
function(){this.svg.resize();this.options.grid&&this.yAxis.tickSize(-this.svg.width);this.x.rangeRoundBands([0,this.svg.width],.1);this.groupedX.rangeRoundBands([0,this.x.rangeBand()],this.options.barSpacing);this.y.range([this.svg.height,0]);this.xAxisSelection.attr("transform","translate(0, "+this.svg.height+")").call(this.xAxis);this.yAxisSelection.call(this.yAxis);this.groups.attr("transform",function(b){return function(d){return"translate("+b.x(d)+", 0)"}}(this));return this.bars.attr("x",function(b){return function(d,
a){return b.groupedX(a)}}(this)).attr("y",function(b){return function(d){return b.y(d)}}(this)).attr("width",this.groupedX.rangeBand()).attr("height",function(b){return function(d){return b.svg.height-b.y(d)}}(this))};a.prototype.draw=function(b){var d,a,c,g;a=function(){var a,c,e,f;e=b.bars;f=[];a=0;for(c=e.length;a<c;a++)d=e[a],f.push("[object Array]"===Object.prototype.toString.call(d)?d:[d]);return f}();this.y.domain([0,d3.max(d3.merge(a,function(b){return d3.max(b)}))]);this.x.domain(b.labels);
this.groupedX.domain(a[0].map(function(b,a){return a}));this.options.margin.left=this.calc.calcLeftMargin(this.yAxis,this.options.margin.left);this.xAxisSelection=this.svg.chart.append("g").attr("class","x axis");this.yAxisSelection=this.svg.chart.append("g").attr("class","y axis");this.groups=this.svg.chart.selectAll(".bar-group").data(b.labels).enter().append("g").attr("class",function(b){return"bar-group "+b});this.bars=this.groups.selectAll(".bar").data(function(b,d){return a[d]}).enter().append("rect").attr("class",
