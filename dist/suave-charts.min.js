...
defaultLineOptions$$module$target$coffeescript$defaults={autoMargins:!0,aspectRatio:"16:9",dotSize:6,grid:!0,xScale:"ordinal",yScale:"linear",smoothLines:!1,stack:!1,tooltips:!0,xLabelFormat:null,yLabelFormat:null,tooltipFormat:function(d){return d3.format(",.0f")(d)},margin:{top:50,bottom:50,right:80,left:80}};
...
var module$target$coffeescript$abstract_chart=AbstractChart$$module$target$coffeescript$abstract_chart=function(){function d(c,b,a){null==b&&(b={});null==a&&(a={});this.options=defaults$$module$target$coffeescript$abstract_chart.mergeOptions(b,a);this.svg=new Svg$$module$target$coffeescript$abstract_chart(c,this.options.aspectRatio,this.options.margin);this.listenerBound=!1;this.axisLabelPadding=10}d.prototype.draw=function(){if(!this.listenerBound)return window.addEventListener("resize",this.render),
this.listenerBound=!0};d.prototype.remove=function(){var c,b;window.removeEventListener("resize",this.render);c=this.svg.domElement;for(b=[];c.firstChild;)b.push(c.removeChild(c.firstChild));return b};return d}();var filterTicks$$module$target$coffeescript$filter_ticks,module$target$coffeescript$filter_ticks=filterTicks$$module$target$coffeescript$filter_ticks=function(d,c,b){var a,k;return"ordinal"===b?(a=c.domain(),k=Math.round(a.length/(d-2)),a.filter(function(b,c){return 0===c||b===a[a.length-1]?!0:0===c%k})):c.ticks(d)};var Axes$$module$target$coffeescript$axes,filterTicks$$module$target$coffeescript$axes,bind$$module$target$coffeescript$axes=function(d,c){return function(){return d.apply(c,arguments)}};filterTicks$$module$target$coffeescript$axes=module$target$coffeescript$filter_ticks;
var module$target$coffeescript$axes=Axes$$module$target$coffeescript$axes=function(){function d(c,b,a,d,f){this.xScale=b;this.yScale=a;this.options=d;this.axisLabelPadding=f;this.newAxis=bind$$module$target$coffeescript$axes(this.newAxis,this);this.xAxisSelection=c.append("g").attr("class","x axis");this.yAxisSelection=c.append("g").attr("class","y axis");this.xAxis=this.newAxis(this.xScale,"bottom");this.yAxis=this.newAxis(this.yScale,"left")}d.prototype.draw=function(c,b){var a;this.xAxisSelection.attr("transform",
...
this.xScale,this.options.xScale),this.xAxis.tickValues(a));this.xAxisSelection.call(this.xAxis);return this.yAxisSelection.call(this.yAxis)};d.prototype.newAxis=function(c,b){return d3.svg.axis().scale(c).orient(b).tickPadding(this.axisLabelPadding)};return d}();var MarginCalculator$$module$target$coffeescript$margin_calculator,module$target$coffeescript$margin_calculator=MarginCalculator$$module$target$coffeescript$margin_calculator=function(){function d(c){this.svg=c}d.prototype.calcLeftMargin=function(c,b,a,d){var f;null==a&&(a="y");null==d&&(d=20);a=this.svg.chart.append("g").attr("class",a+" axis").attr("opacity",0);a.call(c);c=a.selectAll("text").sort(function(a,b){return b.toString().length-a.toString().length});b=1<=c.length&&1<=c[0].length?(f=c[0][0].getBBox(),
...
for(d=k.length;c<d;c++)a=k[c],e.push("[object Array]"===Object.prototype.toString.call(a)?a:[a]);return e}();this.y.domain([0,d3.max(d3.merge(d,function(a){return d3.max(a)}))]);this.x.domain(b.labels);this.groupedX.domain(d[0].map(function(a,b){return b}));null!=this.options.ticks&&0<this.options.ticks&&this.xAxis.tickValues(filterTicks$$module$target$coffeescript$bar_chart(this.options.ticks,this.x,"ordinal"));"vertical"===this.options.layout?this.options.margin.left=this.calc.calcLeftMargin(this.yAxis,
this.options.margin.left):(this.x.rangeRoundBands([0,10],.1),this.options.margin.left=this.calc.calcLeftMargin(this.xAxis,this.options.margin.left,"x"));this.xAxisSelection=this.svg.chart.append("g").attr("class","x axis");this.yAxisSelection=this.svg.chart.append("g").attr("class","y axis");this.groups=this.svg.chart.selectAll(".barGroup").data(b.labels).enter().append("g").attr("class",function(a,b){return"barGroup barGroup-"+b+" "+a});this.bars=this.groups.selectAll(".bar").data(function(a,b){return d[b]}).enter().append("rect").attr("class",
function(a,b){return"bar bar-"+b});this.options.tooltips&&(e=this.options.tooltipFormat,f=this.tip||(this.tip=new Tooltip$$module$target$coffeescript$bar_chart(document)),this.bars.on("mouseover",function(a){f.html(e(a));return f.show(this)}),this.bars.on("mouseout",function(a){return f.hide()}));return this.render()};return c}(AbstractChart$$module$target$coffeescript$bar_chart);var AbstractChart$$module$target$coffeescript$donut_chart,DonutChart$$module$target$coffeescript$donut_chart,defaultDonutOptions$$module$target$coffeescript$donut_chart,bind$$module$target$coffeescript$donut_chart=function(d,c){return function(){return d.apply(c,arguments)}},extend$$module$target$coffeescript$donut_chart=function(d,c){function b(){this.constructor=d}for(var a in c)hasProp$$module$target$coffeescript$donut_chart.call(c,a)&&(d[a]=c[a]);b.prototype=c.prototype;d.prototype=new b;d.__super__=
...
a.barWidth).attr("height",a.barHeight)};c.prototype.draw=function(b){var a,d,f,e;c.__super__.draw.call(this);a=d3.layout.histogram();this.options.domain?(this.x.domain(this.options.domain),this.histogram=a.bins(this.x.ticks(this.options.bins))(b.values),this.xAxis.ticks(this.options.bins)):(this.x.domain(d3.extent(b.values)).nice(),d=this.x.ticks(this.options.bins),this.histogram=a.bins(d)(b.values),this.xAxis.tickValues(d));this.y.domain([0,d3.max(this.histogram,function(a){return a.y})]).nice();
...
return b.area(a.values)}}(this));return this.dots.attr("cx",function(b){return function(a){return b.x(a.x)}}(this)).attr("cy",function(b){return function(a){return b.y(a.y+a.baseline)}}(this))};c.prototype.chooseInterpolation=function(b){return b.smooth?"monotone":"linear"};c.prototype.formatData=function(b,a){var c,d,e,g,v,w,l,h,m,n,y,z,p,q,x,r,t,u;null==a&&(a=!1);v=this.isTimeSeries&&"number"===typeof b.labels[0]?b.labels.map(function(a){return new Date(a)}):b.labels;t=r=p=q=null;m=[];l=b.lines;
e=c=0;for(w=l.length;c<w;e=++c)for(g in h=l[e],m.push({values:Array(h.values.length)}),h)n=h[g],"values"!==g&&(m[e][g]=n);e=n=0;for(w=v.length;n<w;e=++n)for(g=v[e],c=0,z=b.lines,d=y=0,l=z.length;y<l;d=++y){h=z[d];x=h.values[e];u=x+c;if(null==q||q>g)q=g;if(null==p||p<g)p=g;if(null==t||t>u)t=u;if(null==r||r<u)r=u;d=m[d];d.values[e]={x:g,y:x,baseline:c,lineKey:h.label};a&&(c+=x)}return{labels:v,lines:m,xDomain:[q,p],yDomain:["log"===this.options.yScale?t:0,r]}};c.prototype.draw=function(b){var a,d,f;
c.__super__.draw.call(this);d=this.formatData(b,this.options.stack);a=d.labels;b=d.lines;f=d.xDomain;d=d.yDomain;"ordinal"===this.options.xScale?this.x.domain(a):this.x.domain(f);this.y.domain(d).nice();this.line.x(function(a){return function(b){return a.x(b.x)}}(this)).y(function(a){return function(b){return a.y(b.y+b.baseline)}}(this));this.area.x(function(a){return function(b){return a.x(b.x)}}(this)).y0(function(a){return function(b){return a.y(b.baseline)}}(this)).y1(function(a){return function(b){return a.y(b.y+
b.baseline)}}(this));this.options.autoMargins&&(this.options.margin.left=this.calc.calcLeftMargin(this.axes.yAxis,this.options.margin.left));this.lineGroups=this.svg.chart.selectAll(".line").data(b,function(a){return a.label});a=this.lineGroups.enter().append("g").attr("class",function(a,b){return"lineGroup line-"+b+" "+a.label});this.enterLines(a);this.enterAreas(a);this.lines=this.lineGroups.selectAll(".line");this.areas=this.lineGroups.selectAll(".area");this.dotGroups=this.svg.chart.selectAll(".dotGroup").data(b,
