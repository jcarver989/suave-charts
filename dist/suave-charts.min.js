...
100)};d.prototype.draw=function(){if(!this.listenerBound)return window.addEventListener("resize",this.render),this.listenerBound=!0};d.prototype.remove=function(){var c,b;window.removeEventListener("resize",this.render);null!=this.tip&&this.tip.remove();c=this.svg.domElement;for(b=[];c.firstChild;)b.push(c.removeChild(c.firstChild));return b};return d}();var filterTicks$$module$target$coffeescript$filter_ticks,module$target$coffeescript$filter_ticks=filterTicks$$module$target$coffeescript$filter_ticks=function(d,c,b){var a,g;return"ordinal"===b?(a=c.domain(),g=Math.round(a.length/(d-2)),a.filter(function(b,c){return 0===c||b===a[a.length-1]?!0:0===c%g})):c.ticks(d)};var Axes$$module$target$coffeescript$axes,filterTicks$$module$target$coffeescript$axes,bind$$module$target$coffeescript$axes=function(d,c){return function(){return d.apply(c,arguments)}};filterTicks$$module$target$coffeescript$axes=module$target$coffeescript$filter_ticks;
var module$target$coffeescript$axes=Axes$$module$target$coffeescript$axes=function(){function d(c,b,a,g,d){this.xScale=b;this.yScale=a;this.options=g;this.axisLabelPadding=d;this.newAxis=bind$$module$target$coffeescript$axes(this.newAxis,this);this.xAxisSelection=c.append("g").attr("class","x axis");this.yAxisSelection=c.append("g").attr("class","y axis");this.xAxis=this.newAxis(this.xScale,"bottom");this.yAxis=this.newAxis(this.yScale,"left")}d.prototype.draw=function(c,b){var a,g;this.xAxisSelection.attr("transform",
"translate(0,"+b+")");this.options.grid&&(this.xAxis.tickSize(-b,0),this.yAxis.tickSize(-c,0));this.options.xLabelInterval?(g=function(){switch(this.options.xLabelInterval){case "seconds":return d3.time.seconds;case "minutes":return d3.time.minutes;case "hours":return d3.time.hours;case "days":return d3.time.days;case "months":return d3.time.months;case "years":return d3.time.years}}.call(this),a=null!=this.options.tickStepSize?this.options.tickStepSize:1,this.xAxis.ticks(g,a)):null!=this.options.ticks&&
0<this.options.ticks&&(g=filterTicks$$module$target$coffeescript$axes(this.options.ticks,this.xScale,this.options.xScale),this.xAxis.tickValues(g));this.yAxis.ticks(this.options.yTicks);this.xAxisSelection.call(this.xAxis);return this.yAxisSelection.call(this.yAxis)};d.prototype.newAxis=function(c,b){return d3.svg.axis().scale(c).orient(b).tickPadding(this.axisLabelPadding)};return d}();var MarginCalculator$$module$target$coffeescript$margin_calculator,module$target$coffeescript$margin_calculator=MarginCalculator$$module$target$coffeescript$margin_calculator=function(){function d(c){this.svg=c}d.prototype.calcLeftMargin=function(c,b,a,g){var d;null==a&&(a="y");null==g&&(g=20);a=this.svg.chart.append("g").attr("class",a+" axis").attr("opacity",0);a.call(c);c=a.selectAll("text").sort(function(a,b){return b.toString().length-a.toString().length});b=1<=c.length&&1<=c[0].length?(d=c[0][0].getBBox(),
Math.round(d.width)+g):b;a.remove();return b};return d}();var Tooltip$$module$target$coffeescript$tooltip,bind$$module$target$coffeescript$tooltip=function(d,c){return function(){return d.apply(c,arguments)}},module$target$coffeescript$tooltip=Tooltip$$module$target$coffeescript$tooltip=function(){function d(c){this.remove=bind$$module$target$coffeescript$tooltip(this.remove,this);this.hide=bind$$module$target$coffeescript$tooltip(this.hide,this);this.show=bind$$module$target$coffeescript$tooltip(this.show,this);this.html=bind$$module$target$coffeescript$tooltip(this.html,
...
a,c,d,e,h,p,l;this.svg.resize();this.x.rangeRoundBands([0,this.svg.width]);a={xTransform:"translate(0, "+this.svg.height+")",yTransform:"",groupsTransform:function(a){return function(b){return"translate("+a.x(b)+", 0)"}}(this),xRange:[0,this.svg.width],yRange:[this.svg.height,0],barX:function(a){return function(b){return a.x(b.label)}}(this),barY:function(a){return function(b){return a.y(b.value)}}(this),barWidth:function(a){return function(){return a.x.rangeBand()}}(this),barHeight:function(a){return function(b){return a.svg.height-
...
b.tipPadding)}}(this)).text(function(a){return function(b){return a.options.tooltipFormat(b.value)}}(this));h=this.totalBar;p=this.totalBarTooltip;l=this.y;c=this.sum;e=this.options.tooltipFormat;d=this.tipPadding;b=d3.behavior.drag().on("drag",function(b){var f,k,n;f=c();f=Math.min(Math.round(l.invert(d3.event.y)-b.value),f);f=Math.max(0,Math.min(b.value+f,l.domain()[1]));b.value=f;d3.select(this).attr("cy",l(f));n=d3.select(this.parentNode);n.select("rect").attr("height",a.barHeight({value:f})).attr("y",
a.barY({value:f}));n.select("text").attr("y",function(b){return a.barY(b)-d}).text(e(f));k=c();h.each(function(a){return a.value=k});h.attr("height",a.barHeight({value:k})).attr("y",a.barY({value:k}));p.attr("y",a.barY({value:k})-d).text(e(k));return this.dispatchEvent(new CustomEvent("goalChartChanged",{detail:b,bubbles:!0}))});return this.dots.attr("cx",function(b){return a.barX(b)+.5*a.barWidth()}).attr("cy",function(b){return a.barY(b)+2.5}).attr("class","drag-handle").call(b)};c.prototype.draw=
...
return b.area(a.values)}}(this));return this.dots.attr("cx",function(b){return function(a){return b.x(a.x)}}(this)).attr("cy",function(b){return function(a){return b.y(a.y+a.yTranslation)}}(this))};c.prototype.chooseInterpolation=function(b){return b.smooth?"monotone":"linear"};c.prototype.formatData=function(b,a){var c,d,e,h,p,l,m,f,k,n,y,z,q,r,x,t,u,v,w;null==a&&(a=!1);p=this.isTimeSeries&&"number"===typeof b.labels[0]?b.labels.map(function(a){return new Date(a)}):b.labels;u=t=q=r=null;k=[];m=b.lines;
e=c=0;for(l=m.length;c<l;e=++c)for(h in f=m[e],k.push({values:Array(f.values.length)}),f)n=f[h],"values"!==h&&(k[e][h]=n);e=n=0;for(l=p.length;n<l;e=++n)for(h=p[e],v=c=0,z=b.lines,d=y=0,m=z.length;y<m;d=++y){f=z[d];x=f.values[e];w=x+v;if(null==r||r>h)r=h;if(null==q||q<h)q=h;if(null==u||u>w)u=w;if(null==t||t<w)t=w;d=k[d];null!=f.baselineValues&&(c=f.baselineValues[e]);d.values[e]={x:h,y:x,yTranslation:v,areaY0:c,lineKey:f.label};a&&(v+=x)}return{labels:p,lines:k,xDomain:[r,q],yDomain:["log"===this.options.yScale?
