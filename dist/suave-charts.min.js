...
var module$target$coffeescript$axes=Axes$$module$target$coffeescript$axes=function(){function d(c,b,a,d,v){this.xScale=b;this.yScale=a;this.options=d;this.axisLabelPadding=v;this.newAxis=bind$$module$target$coffeescript$axes(this.newAxis,this);this.xAxisSelection=c.append("g").attr("class","x axis");this.yAxisSelection=c.append("g").attr("class","y axis");this.xAxis=this.newAxis(this.xScale,"bottom");this.yAxis=this.newAxis(this.yScale,"left")}d.prototype.draw=function(c,b){var a,d;this.xAxisSelection.attr("transform",
...
0<this.options.ticks&&(d=filterTicks$$module$target$coffeescript$axes(this.options.ticks,this.xScale,this.options.xScale),this.xAxis.tickValues(d));this.yAxis.ticks(this.options.yTicks);this.xAxisSelection.call(this.xAxis);return this.yAxisSelection.call(this.yAxis)};d.prototype.newAxis=function(c,b){return d3.svg.axis().scale(c).orient(b).tickPadding(this.axisLabelPadding)};return d}();var MarginCalculator$$module$target$coffeescript$margin_calculator,module$target$coffeescript$margin_calculator=MarginCalculator$$module$target$coffeescript$margin_calculator=function(){function d(c){this.svg=c}d.prototype.calcLeftMargin=function(c,b,a,d){var v;null==a&&(a="y");null==d&&(d=20);a=this.svg.chart.append("g").attr("class",a+" axis").attr("opacity",0);a.call(c);c=a.selectAll("text").sort(function(a,b){return b.toString().length-a.toString().length});b=1<=c.length&&1<=c[0].length?(v=c[0][0].getBBox(),
Math.round(v.width)+d):b;a.remove();return b};return d}();var Tooltip$$module$target$coffeescript$tooltip,bind$$module$target$coffeescript$tooltip=function(d,c){return function(){return d.apply(c,arguments)}},module$target$coffeescript$tooltip=Tooltip$$module$target$coffeescript$tooltip=function(){function d(c){this.remove=bind$$module$target$coffeescript$tooltip(this.remove,this);this.hide=bind$$module$target$coffeescript$tooltip(this.hide,this);this.show=bind$$module$target$coffeescript$tooltip(this.show,this);this.html=bind$$module$target$coffeescript$tooltip(this.html,
...
return b.area(a.values)}}(this));return this.dots.attr("cx",function(b){return function(a){return b.x(a.x)}}(this)).attr("cy",function(b){return function(a){return b.y(a.y+a.yTranslation)}}(this))};c.prototype.chooseInterpolation=function(b){return b.smooth?"monotone":"linear"};c.prototype.formatData=function(b,a){var c,d,e,f,w,x,k,g,l,m,z,A,n,p,y,q,r,t,u;null==a&&(a=!1);w=this.isTimeSeries&&"number"===typeof b.labels[0]?b.labels.map(function(a){return new Date(a)}):b.labels;r=q=n=p=null;l=[];k=b.lines;
e=c=0;for(x=k.length;c<x;e=++c)for(f in g=k[e],l.push({values:Array(g.values.length)}),g)m=g[f],"values"!==f&&(l[e][f]=m);e=m=0;for(x=w.length;m<x;e=++m)for(f=w[e],t=c=0,A=b.lines,d=z=0,k=A.length;z<k;d=++z){g=A[d];y=g.values[e];u=y+t;if(null==p||p>f)p=f;if(null==n||n<f)n=f;if(null==r||r>u)r=u;if(null==q||q<u)q=u;d=l[d];null!=g.baselineValues&&(c=g.baselineValues[e]);d.values[e]={x:f,y:y,yTranslation:t,areaY0:c,lineKey:g.label};a&&(t+=y)}return{labels:w,lines:l,xDomain:[p,n],yDomain:["log"===this.options.yScale?
